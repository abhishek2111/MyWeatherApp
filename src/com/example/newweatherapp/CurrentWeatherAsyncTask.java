...
public class CurrentWeatherAsyncTask extends
	...
	public CurrentWeatherAsyncTask(Context ctx, View view) {
		context = ctx;
		rootview = view;
	}
	...
	protected Boolean doInBackground(Boolean... params) {
		...
		if (isInCache) {
			...
			data = SetConnection.getcurrentweatherdata(context,
					CityPreference.getcity((Activity) context));
		}
	...
	private boolean getdatafromString(String data) {
			...
			try {
				...
				if (jobj.getInt("cod") == 200) {
					RawCache.cache(context,
							CityPreference.getcity((Activity) context)
									+ Utilities.CURRENT_CACHE_KEY, data);
	...
	private void updatetheweather() {
		...
		try {
			...
			Weather_icon.setImageResource(Utilities.getImageDrawable(iconid,
					sunrise, sunset));

		} catch (Exception e) {
			Toast.makeText(
					context,
					"Data not received properly.Please check your internet connection.",
